...
int write_kline(Kline *data, FILE *output_file) {
  int ret = fprintf(output_file, "%ld,%ld,%f,%f,%f,%f,%f,%f,%d,%d\n", data -> open_ts,
                    data -> close_ts, data -> open, data -> high, data -> low,
                    data -> close, data -> buy_vol_usdt, data -> sell_vol_usdt, data -> buy_trades, data -> sell_trades);
  return ret;
}

// write the file header
int write_header(FILE *output_file) {
  int ret = fprintf(output_file, "open_ts,close_ts,open,high,low,close,buy_volume_usdt,sell_volume_usdt,buy_trades_count,sell_trades_count\n");
  return ret;
}

// utility function to print a Kline candle
void print_candle(Kline *candle) {
  printf("OPEN_TS       CLOSE_TS      OPEN      HIGH      LOW       CLOSE     BUY_VOL        SELL_VOL\n");
  printf("%ld %ld %f %f %f %f %f %f\n", candle -> open_ts, candle -> close_ts, candle -> open, candle -> high, candle -> low, candle -> close, candle -> buy_vol_usdt, candle -> sell_vol_usdt);
  printf("-----------------------------------------------------\n");
}
...
unsigned long next_timestamp(unsigned long timestamp, unsigned int seconds) {
    return ((timestamp + (seconds * 1000) - 1) / (seconds * 1000)) * (seconds * 1000);
}
